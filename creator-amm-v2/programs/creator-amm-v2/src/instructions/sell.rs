...
pub fn handler(ctx: Context<Sell>, base_amount: u64, min_quote_amount: u64) -> Result<()> {
    ...
    if pool.is_anti_sniper_active(clock.slot, config.anti_sniper_window_slots) {
        ...
        require!(base_amount <= max, ErrorCode::AntiSniperActive);
    }

    let quote_output = pool.calculate_output(base_amount, base_res, quote_res, fee_bps)?;
    require!(quote_output >= min_quote_amount, ErrorCode::SlippageExceeded);
    require!(quote_output >= 1000, ErrorCode::OutputTooSmall);
    require!(quote_output <= pool.real_quote_reserves, ErrorCode::InsufficientLiquidity);

    let quote_before_fee = pool.calculate_output(base_amount, base_res, quote_res, 0)?;
    let fee_amount = quote_before_fee.checked_sub(quote_output).ok_or(ErrorCode::MathOverflow)?;

    // Transfer base from user to pool
    ...
    // Transfer quote from pool to user
    ...
    if matches!(pool.current_phase, CurvePhase::Graduated) {
        pool.real_base_reserves = pool.real_base_reserves.checked_add(base_amount).ok_or(ErrorCode::MathOverflow)?;
        pool.real_quote_reserves = pool.real_quote_reserves.checked_sub(quote_output).ok_or(ErrorCode::MathOverflow)?;
    } else {
        pool.virtual_base_reserves = pool.virtual_base_reserves.checked_add(base_amount).ok_or(ErrorCode::MathOverflow)?;
        pool.virtual_quote_reserves = pool.virtual_quote_reserves.checked_sub(quote_before_fee).ok_or(ErrorCode::MathOverflow)?;
        pool.real_base_reserves = pool.real_base_reserves.checked_add(base_amount).ok_or(ErrorCode::MathOverflow)?;
        pool.real_quote_reserves = pool.real_quote_reserves.checked_sub(quote_before_fee).ok_or(ErrorCode::MathOverflow)?;
    }
    ...
    emit!(TradeExecuted {
        ...
        is_buy: false, input_amount: base_amount, output_amount: quote_output,
        fee_amount, fee_bps, phase: pool.current_phase,
