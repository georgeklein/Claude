...
pub struct Sell<'info> {
    ...
    #[account(
        ...
        bump = pool.bump,
        constraint = !pool.graduated @ ErrorCode::PoolGraduated,
    )]
    ...
    /// User's quote token account (receiving quote tokens)
    #[account(
        mut,
    ...
    /// User's base token account (paying with base tokens)
    #[account(
        mut,
...
pub fn handler(
    ...
    // Anti-sniper check
    if pool.is_anti_sniper_active(clock.slot, config.anti_sniper_window) {
        // During anti-sniper window, limit trade size
    ...
    // Calculate output amount using bonding curve with fee
    ...
    // Calculate protocol fee (taken from output)
    let fee_amount = (quote_output as u128)
        .checked_mul(config.protocol_fee_bps as u128)
        .ok_or(ErrorCode::MathOverflow)?
        .checked_div(10000)
        .ok_or(ErrorCode::MathOverflow)? as u64;

    let quote_to_user = quote_output.checked_sub(fee_amount)
        .ok_or(ErrorCode::MathOverflow)?;
    ...
    msg!("Sell executed!");
    msg!("Base in: {}", base_amount);
    msg!("Quote out: {}", quote_to_user);
    msg!("Fee: {}", fee_amount);
    msg!("New virtual reserves: {} quote, {} base", pool.virtual_quote_reserves, pool.virtual_base_reserves);
