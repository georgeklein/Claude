...
pub struct Buy<'info> {
    ...
    /// User's quote token account (paying with quote tokens)
    #[account(
        mut,
    ...
    /// User's base token account (receiving base tokens)
    #[account(
        mut,
...
pub fn handler(
    ...
    let quote_to_pool = quote_amount.checked_sub(fee_amount)
        .ok_or(ErrorCode::MathOverflow)?;
    ...
    msg!("Buy executed!");
    msg!("Quote in: {}", quote_amount);
    msg!("Base out: {}", base_output);
    msg!("Fee: {}", fee_amount);
